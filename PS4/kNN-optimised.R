...
kNN <- function(features, labels, memory=NULL, k = 1, p = 2, type = 'train') {
    ...
    if (type == "train") {
        ...
        for (obs in 1:noObs) {
            ...
            probeExpanded <- matrix(probe, nrow = noObs, ncol = noVars, byrow = TRUE) # potentially cut this down a bit, but worth it?
        
            # computing distances between the probe and exemplars in the
            # training X
            # Only calculate half of the matrix and copy the other half (since it's symmetric)
            if (p %in% c(1,2)) {
